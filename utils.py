...
import random
from model import *
from load_data import load_data, trim_outliers


def run_inference(
    predictor: ElectronOpticsPredictor = None,
    batch_size: int = 32,
    use_training_ds: bool = False,
    shuffle: bool = True,
...
def trim_hist(
    ...
    **kwargs: Any,
    ...
    voltages, output_values, outlier_voltages, outlier_outputs = trim_outliers(
        voltages_unclean,
        output_values_unclean,
...
def trim_scatter(
    data_paths: list[str],
    voltages_start: int = None,
    voltages_end: int = 13,
    output_values_start: int = 14,
    output_values_end: int = None,
    trim_mode: str = "z-score",
    ...
    for v in range(output_values.shape[1]):
        ...
        output_value = np.random.permutation(
            output_value
        )  # Randomize order for better visibility
        ax[v].scatter(
            np.arange(len(output_values)),
            output_value,
            label="full",
            color="k",
            s=100,
    ...
    for v in range(output_values_masked.shape[1]):
        ...
        output_value_masked = np.random.permutation(
            output_value_masked
        )  # Randomize order for better visibility
        ax[v].scatter(
            np.arange(len(output_values_masked)),
            output_value_masked,
            label="trimmed",
            color="b",
            s=10,
